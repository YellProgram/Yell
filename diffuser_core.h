...
public:
	...
	Grid(cctbx::uctbx::unit_cell _cell,
       vec3<double> _grid_steps, 
       vec3<double> _lower_limits,
       vec3<int> _grid_size = vec3<int>(1,1,1),
       bool _reciprocal_flag = true) : // by default the grid is defined in reciprocal space. Usually we start working with diffuse scattering
	cell(_cell), grid_steps(_grid_steps), lower_limits(_lower_limits), grid_size(_grid_size), reciprocal_flag(_reciprocal_flag)
	{}
  ...
  /// Returns corresponding grid in reciprocal space.
	Grid reciprocal()	{
		vec3<double> _grid_steps(1,1,1);
		vec3<double> _lower_limits(0,0,0);
		for(int i=0; i<3; i++)
		{
			//cout << "lower limit " << i << " " << lower_limits[i];
			if(abs(lower_limits[i])!=0)
			{
				_grid_steps[i]=-0.5/lower_limits[i];
				_lower_limits[i]=-0.5/grid_steps[i];
			}
		}
		
		return Grid(cell.reciprocal(),_grid_steps,_lower_limits,grid_size,!reciprocal_flag);
	}
  
  /// Function for unit tests. 
  bool operator==(const Grid& inp)  {
    return almost_equal(grid_steps,inp.grid_steps) && almost_equal(lower_limits, inp.lower_limits) && almost_equal(cell.parameters(),inp.cell.parameters()) && grid_size==inp.grid_size;
  }
